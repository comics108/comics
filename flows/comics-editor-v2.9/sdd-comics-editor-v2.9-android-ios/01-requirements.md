...
- [x] **Q1. Слой данных на мобильных**: изначально утверждён (2026-07-23) **.NET NativeAOT + FFI** — C#-ядро компилируется в нативную библиотеку с C-экспортами (`UnmanagedCallersOnly`), вызовы через `dart:ffi`. **По факту неприменимо ни к iOS, ни к Android** — оба архитектурно на **Mono**, не CoreCLR, и CoreCLR NativeAOT явно исключает мобильные RID в .NET 10 (`Microsoft.NETCoreSdk.BundledVersions.props`: `RuntimePackExcludedRuntimeIdentifiers="android;linux-bionic"`; `ILCompilerRuntimeIdentifiers` не содержит ни ios-, ни bionic/android-RID). На обеих платформах активирован предусмотренный в этом же Q1 **fallback: чистый Dart** (`package:archive`, тот же протокол/методы, что у остальных ядер) — `DartIoCore` в `lib/src/bridge/dart_io_core.dart`. Десктоп (Windows/macOS/Linux) продолжает работать через headless-процесс (`CoreClient`), не затронут.
  - **iOS (2026-07-23)**: после установки `sudo dotnet workload install ios` выяснилось, что этот workload даёт Mono-AOT пайплайн для App-бандлов (`Microsoft.iOS.Sdk`), а не голые библиотеки с C-экспортами; пакет `Microsoft.DotNet.ILCompiler.LLVM` не публикуется на nuget.org. Подробности — `04-implementation-log.md`, Session 2.
  - **Android (2026-07-25)**: та же ошибка сборки (`PrivateSdkAssemblies ItemGroup is required`), сначала ошибочно списанная на невозможность кросс-компиляции NativeAOT с macOS на Linux — но воспроизвелась и **на настоящем Ubuntu CI-раннере**. Прямая проверка метаданных SDK показала: причина не в хосте сборки, а в том, что Android/bionic исключены из NativeAOT runtime pack самим SDK. Подробности — `04-implementation-log.md`, Session 3. Пользователь подтвердил тот же fallback, что и для iOS.
  - `FfiCore`/`Comics.Editor.Native` не удалены — архивный, протестированный на macOS задел на случай будущей публичной поддержки мобильного NativeAOT.
