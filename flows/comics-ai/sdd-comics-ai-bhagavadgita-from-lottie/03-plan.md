...
- **Description**: Implement `scene_pan`, `frame_to_scroll_y` (using the real, already-verified
  `st`/`sr`-based local→root frame conversion), `extract_layer_motion`, `to_translate_anim_keyframes`,
  and `derive_z_depth` per `02-specifications.md`, using Task 1.1's confirmed (or corrected)
  compositing formula.
- **Files**:
...
- **Verification**: unit tests against the real file's real numbers cited in Requirements/
  ...
  values and derived z-depth); a real division-by-zero guard test for the degenerate case
...
- **Description**: Implement `select_camera_reference_layer` (scale-presence > keyframe-count >
  ...
  `build_camera_path` (reuses `to_translate_anim_keyframes` against the selected layer). Update
...
- **Verification**: unit test confirming `select_camera_reference_layer` picks the real `comp_0`
  `ind=43` layer on the real data; unit test confirming `build_camera_path`'s output matches
  `to_translate_anim_keyframes`'s own output for that same layer (no drift between the two
  representations); integration test confirming the real `cameraPath` is NOT identical to the
...
- **Description**: `PackagingAsset`/`_layer_json` (from `sdd-comics-ai-bhagavadgita-generator`)
  ...
  and write an optional document-root `cameraPath` list (sibling to `layers`/`sounds`).
- **Files**:
...
- **Verification**: `sdd-comics-ai-bhagavadgita-generator`'s existing Phase 6 tests (18-chapter
  ...
  and for `cameraPath` being written/omitted correctly at the document root
- **Complexity**: Medium
...
- [x] Approved on: 2026-08-09 — approved in the parent flow as v0.3 ("reqs,specs and plan approved")
      before extraction.
- [x] Notes: Task 1.1 is deliberately the *first* implementation step precisely because real
      engineering risk remains open (compositing formula, camera-reference-layer selection
      heuristic, the `K` constant, cross-flow schema adoption) — approval doesn't collapse those into
      "resolved."
