# Status: tdd-dot-bodymovin-format
...
(2026-08-07), after Anton confirmed this really is a distinct format (Bodymovin/Bodymovin) and renamed
the fixtures accordingly (`samples/sample_v2026.comics`→`sample.Bodymovin`,
`dataset/mahabharata/boranko/mahabharata-dot-comics_v2026`→`mahabharata-dot-bodymovin`). Revised same
day: Anton pointed out the vendored Bodymovin engine is also in `legacy/mahabharata-mobile-swift-v2012`
(not just the 2026 copy), which changes the history significantly — see Discoveries. Also answered
two direct questions: how Bodymovin was used in legacy v2012 Java (it wasn't, at all), and whether
`.Bodymovin ↔ .comics` conversion is feasible via simple math (yes one direction, conditionally the
...
- **Resolved, 2026-08-07 (in a sibling flow)**: the precomp-nesting question this flow raised (does
  `.comics` need a grouping concept to represent Bodymovin's nested compositions) has been answered —
  see `flows/comics-editor/tdd-dot-bodymovin-import-export/01-requirements.md`'s Precomp Handling
...
- [x] Requirements revised (2026-08-07) — v1.1: corrected the v2012-vs-v2026 Bodymovin history, added
      the "Bodymovin in Android v2012" answer and the conversion-feasibility analysis
...
1. **Corrected history**: the vendored, unused `LOT*` Bodymovin engine is not a 2026 addition — it's
...
2. **The one real Bodymovin usage (the "now playing" equalizer icon, `AnimationView(name:
   "equalizer")`, a 14×14px animation) also dates to 2012**, using a separate mechanism (the modern
   `bodymovin-ios` CocoaPod) from the dead vendored engine.
3. **Android has never had Bodymovin, in any generation** — re-confirmed directly against
...
4. **Conversion feasibility, grounded in real data — for `ASHES.json` specifically.**
   `ASHES.json`'s 101 layers are 100% Bodymovin image type (`ty:2`) — zero shapes/masks/text — and
   position/scale keyframes consistently use After Effects' default "Easy Ease" bezier handles
   (`{0.833,0.833}`/`{0.167,0.167}`), not arbitrary curves. This makes `.comics → .Bodymovin`
   straightforwardly mechanical for this file, and would make `.Bodymovin → .comics` conditionally
...
5. **CORRECTION (2026-08-07): does NOT generalize.** Checked all 7 real chapters directly: `THE
   ...
   **and** 190/295 layers (64%) using Bodymovin's `parent` field (transform-relative-to-another-layer)
...
- **Why this is its own flow, not a section of `tdd-dot-comics-format`**: Bodymovin is not a version
...
- **The naming correction matters for anyone reading old context**: earlier in this research
  ...
  this immediately — it's genuinely Bodymovin, and the fixtures were renamed to reflect that
  (`.Bodymovin` extension, `mahabharata-dot-bodymovin` directory name, not `_v2026`). All paths in this
  flow's docs use the corrected names.
- **A real, non-obvious finding preserved here**: `apps/mahabharata-mobile-swift-v2026` has a
  complete, real, vendored Bodymovin rendering engine that's never actually instantiated anywhere in
...
- Reason: Anton confirmed the discovery was a genuinely separate format (Bodymovin), not a `.comics`
...
- Changes: all path references corrected from the original (mislabeled) `sample_v2026.comics`/
  `mahabharata-dot-comics_v2026` names to the renamed `sample.Bodymovin`/`mahabharata-dot-bodymovin`;
...
3. Confirm with whoever owns the Swift app (2012 and/or 2026 copy) whether the vendored Bodymovin
   engine integration is in progress elsewhere, or genuinely not started, and why it's been carried
   forward unused since 2012.
4. On direction from Anton: either proceed toward Specifications (if Bodymovin is a committed
   direction worth building test infrastructure for, possibly including a real `.comics ↔ .Bodymovin`
