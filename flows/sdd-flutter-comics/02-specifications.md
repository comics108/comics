...
portable `.Bodymovin` parsing/import/export code, relocated verbatim from `.../lib/src/bridge/
bodymovin_mapping.dart` + `.../lib/src/ui/bodymovin/bodymovin_import.dart`/`bodymovin_export.dart`; (d) a new
...
Two changes of substance: (1) the Bodymovin files + interpolator are now in scope (see
...
├── Bodymovin         BodymovinDocument/BodymovinLayer/BodymovinAsset/BodymovinMask/BodymovinTransform/
│                  BodymovinProperty/BodymovinKeyframe/BodymovinFormatException, parseBodymovinDocument/
│                  parseBodymovinJson/writeBodymovinDocument (relocated from bodymovin_mapping.dart);
│                  ExportImportMode/ImportPreview/LayerPreview/LayerPreviewStatus/EasingChoice/
│                  detectMode/commitImport (relocated from bodymovin_import.dart); buildBodymovinExport
│                  (relocated from bodymovin_export.dart)
...
apps/comics-editor's own remaining, UNCHANGED-logic, native-core-coupled layer:
  ...
  controller.dart, bodymovin_import_dialog.dart, tile_writer.dart, language_registry.dart
...
| `apps/comics-editor/lib/src/bridge/bodymovin_mapping.dart` | Delete (moved) | → `libs/flutter_comics/lib/src/bodymovin/bodymovin_mapping.dart`. Its `import 'package:archive/archive_io.dart'` should become `package:archive/archive.dart` (only `Archive`/`ZipDecoder`/`ArchiveFile`/`ZipEncoder` are used — none of `archive_io.dart`'s extra IO-file helpers — confirmed by grep; the narrower import is the more genuinely portable one for a library that must also work on Web) |
| `apps/comics-editor/lib/src/ui/bodymovin/bodymovin_import.dart` | Delete (moved) | → `libs/flutter_comics/lib/src/bodymovin/bodymovin_import.dart`, verbatim (`flutter/widgets.dart` for `Offset` + the two files above — confirmed portable, no `dart:io`/`EditorController`) |
| `apps/comics-editor/lib/src/ui/bodymovin/bodymovin_export.dart` | Delete (moved) | → `libs/flutter_comics/lib/src/bodymovin/bodymovin_export.dart`, verbatim (confirmed portable) |
...
| `apps/comics-editor/lib/**` — 15 files import `models.dart` directly (`keyframe_interpolator.dart`, `sound_player.dart`, `controller.dart`, `edit_history.dart`, `bodymovin_export.dart`, `bodymovin_import.dart`, `editor_screen.dart`, `balloon_editor_card.dart`, `balloon_rail.dart`, `dialogs.dart`, `properties_panel.dart`, `scene_panel.dart`, `timeline.dart`, `top_bar.dart`, `viewer_workspace.dart` — exact list, verified by grep, supersedes v0.1's approximate "16 files"); 3 files import `models_mapping.dart` directly (`controller.dart`, `balloon_editor_card.dart`, `cutting_canvas.dart` — supersedes v0.1's approximate "6 files") | Modify | Import path updates only, no logic changes. The 5 files being *moved* (`models.dart`, `keyframe_interpolator.dart`, and the 3 Bodymovin files) drop out of this list once moved — their own former importers just get a new import path |
| `apps/comics-editor/lib/src/ui/widgets/bodymovin_import_dialog.dart`, `EditorController`'s Bodymovin methods in `controller.dart` (`pickBodymovinToImport`/`setBodymovinImportMode`/`setBodymovinScrollSpeed`/`setBodymovinEasingChoice`/`cancelBodymovinImport`/`commitBodymovinImport`/`exportBodymovinWithDialog`) | Modify (import path only) | **Do NOT move** — `file_picker`/`EditorScope`/`CoreDocument.tempFolder`/`writeTiles` (`dart:io`)-coupled UI glue, confirmed by reading their actual imports. Only their `import '.../bodymovin_import.dart'`-style paths change to the package |
...
| `apps/comics-editor/test/bodymovin_mapping_test.dart`, `bodymovin_import_test.dart`, `bodymovin_export_test.dart`, `bodymovin_commit_import_test.dart` | Delete (moved) | → `libs/flutter_comics/test/`. Confirmed by their actual imports: only `bodymovin_mapping.dart`/`bodymovin_import.dart`/`bodymovin_export.dart`/`models.dart` — clean move |
| `apps/comics-editor/test/bodymovin_roundtrip_test.dart` | Delete (moved), **contingent** | → `libs/flutter_comics/test/`, but its G3 case currently imports `models_mapping.dart` for one thing only: parsing a real `.comics` fixture (`comicsFromCore`) as its Full-Canvas-fixture-prep step. **Must be rewritten to use the new `ComicsArchiveReader` instead before it can move** — otherwise it's the one Bodymovin test with a real backwards dependency on `apps/comics-editor`. Sequencing note for Plan: this specific test can only move *after* `comics_reader.dart` exists |
...
| **`apps/comics-editor/test/bodymovin_controller_test.dart`** | **Stays (new in v0.2)** | Tests `EditorController`'s file-picker/tempFolder-writing wiring (`commitBodymovinImport`, etc.) — inherently editor-specific, confirmed by its `controller.dart` import. Not a candidate; wasn't in v0.1 since this test didn't exist yet when v0.1 was drafted |
...
After:
  ...
  libs/flutter_comics/lib/src/bodymovin/bodymovin_mapping.dart (NEW in v0.2 -- portable .Bodymovin ZIP+JSON,
                                                            relocated verbatim)
  libs/flutter_comics/lib/src/bodymovin/bodymovin_import.dart  (NEW in v0.2 -- ImportPreview/commitImport,
                                                            relocated verbatim)
  libs/flutter_comics/lib/src/bodymovin/bodymovin_export.dart  (NEW in v0.2 -- buildBodymovinExport,
                                                            relocated verbatim)
  ...
  apps/comics-editor/.../bodymovin_import_dialog.dart, controller.dart's Bodymovin methods
...
export 'src/bodymovin/bodymovin_mapping.dart';  // BodymovinDocument/BodymovinLayer/BodymovinAsset/BodymovinMask/
                                           // BodymovinTransform/BodymovinProperty/BodymovinKeyframe/
                                           // BodymovinFormatException, parseBodymovinDocument/
                                           // parseBodymovinJson/writeBodymovinDocument (NEW in v0.2)
export 'src/bodymovin/bodymovin_import.dart';   // ExportImportMode/ImportPreview/LayerPreview/
                                           // LayerPreviewStatus/EasingChoice/detectMode/
                                           // commitImport (NEW in v0.2)
export 'src/bodymovin/bodymovin_export.dart';   // buildBodymovinExport (NEW in v0.2)
...
and import/export.** The one place this needed a concrete (not just stylistic) check: `bodymovin_export
.dart` calls `KeyframeInterpolator.translateAt` (confirmed via its own `import`) to sample a layer's
absolute position at export time. Since `bodymovin_export.dart` is unambiguously import/export logic
...
other arrow in this design) or `bodymovin_export.dart` would need a second copy of the same math.
...
- [ ] `libs/flutter_comics/test/bodymovin_mapping_test.dart`, `bodymovin_import_test.dart`,
      `bodymovin_export_test.dart`, `bodymovin_commit_import_test.dart` (relocated, NEW in v0.2) —
...
- [ ] `libs/flutter_comics/test/bodymovin_roundtrip_test.dart` (relocated, NEW in v0.2) — **contingent
...
per the correction above), verify editor suite green, (3) move the 3 Bodymovin files +
`bodymovin_mapping_test.dart`/`bodymovin_import_test.dart`/`bodymovin_export_test.dart`/
`bodymovin_commit_import_test.dart`, update `apps/comics-editor`'s Bodymovin-UI imports
(`bodymovin_import_dialog.dart`, `controller.dart`), verify editor suite green again, (4) write
`comics_reader.dart` + its new test + move `bodymovin_roundtrip_test.dart` (now that
...
- [ ] Exact `libs/flutter_comics/pubspec.yaml` shape — plain Dart package vs. a Flutter package (the
      ...
      `Offset`). `keyframe_interpolator.dart` and `bodymovin_export.dart` both use plain `dart:ui`
      already (not `flutter/widgets.dart`) — only `models.dart` and `bodymovin_import.dart` pull in
...
- [ ] **NEW (v0.2)**: `package:archive/archive_io.dart` → `package:archive/archive.dart` in
      `bodymovin_mapping.dart` — confirmed by grep that only `Archive`/`ZipDecoder`/`ArchiveFile`/
