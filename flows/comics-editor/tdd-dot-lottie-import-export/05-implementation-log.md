# Implementation Log: dot-lottie-import-export
...
| 2.1 `LottieDocument`/`LottieLayer`/`LottieAsset`/`LottieMask`/`LottieTransform` | Done | `lottie_mapping.dart` (new) |
| 2.2 `parseLottieDocument` | Done | Zip via `package:archive` (matches `dart_io_core.dart`'s own pattern) |
| 2.3 `writeLottieDocument` | Done | Inverse; reconstructs the real `NAME_content/NAME.json` layout |
| 3.1 `ExportImportMode` + detection heuristic | Done | `lottie_import.dart` (new); aspect-ratio + real-sweep-shape signal |
...
| 4.1 `commitImport` — Full Canvas | Done | `lottie_import.dart`; bakes precomp offset into members |
...
| 4.4 `TextRegion` import | Done | Lottie vector mask -> `TextRegion.shape:"polygon"` |
| 5.1 `buildLottieExport` — Full Canvas | Done | `lottie_export.dart` (new); identity, inverse of Task 4.1 |
| 5.2 `buildLottieExport` — Playback Viewport | Done | Synthesized `-scrollPixel` sweep per scene, matches `.comics`'s own render model |
...
| 6.1 Menu entries | Done | `top_bar.dart`, new "Lottie import/export" popup (desktop) + 2 items in the compact overflow menu |
| 6.2 Review screen widget | Done | `lottie_import_dialog.dart` (new): mode toggle, scrollSpeed/easing controls, clean/flagged counts, scrollable per-layer list, Cancel/Import |
| 6.3 Wrong-mode display | Done | Same file: banner shown when `preview.mode != detectMode(preview.document)` |
| 6.4 Real image-byte extraction | Done | `EditorController.commitLottieImport`; base64 data URI -> `writeTiles`, external-file-ref case disclosed/skipped |
| 7.1 E1 round-trip | Done | `test/lottie_roundtrip_test.dart`, real `samples/sample.lottie` |
| 7.2 G3 Full Canvas round-trip | Done | Real `samples/sample_v2012.comics_unzip`, `.lottie→.comics→.lottie` direction |
| 7.3 G6 Playback Viewport round-trip | Done | Real `samples/sample_playback_viewport.lottie_unzip` (`ASHES.json`) |
...
**Context**: Plan approved same session. Verified before starting: zero `lottie_*.dart` files
...
- Task 2.1: `LottieDocument`/`LottieLayer`/`LottieAsset`/`LottieMask`/`LottieTransform`/
  `LottieProperty`/`LottieKeyframe`/`AssetFile` — minimal Lottie model, matching Specifications'
  "only what real content needs." **Verified `LottieMask`'s simplified shape against real data
  ...
  never inverted. `LottieMask.fromJson`-equivalent throws `LottieFormatException` for any shape
  outside that (animated path, curved handles) rather than silently guessing.
  - Files changed: `lib/src/bridge/lottie_mapping.dart` (new)
  - Verified by: `test/lottie_mapping_test.dart`
- Task 2.2: `parseLottieDocument(Uint8List zipBytes)` — unzips via `package:archive`'s `ZipDecoder`
  ...
  `LottieFormatException` (typed, not generic `FormatException`) on missing top-level keys or a
  non-JSON content entry — Test F1.
  - Files changed: `lib/src/bridge/lottie_mapping.dart`
  - Verified by: `test/lottie_mapping_test.dart`, including a real end-to-end parse of
    `samples/sample.lottie` (a real zip, not a directory fixture) and real direct-JSON parses of
    `ASHES.json`/`THE CHASE.json` (via the internally-factored pure `parseLottieJson`, which
    doesn't need a zip step to test against real fixtures read directly)
- Task 2.3: `writeLottieDocument` — the inverse, reconstructing the same `NAME_content/NAME.json`
  layout on the way out via `LottieDocument.contentBaseName` (captured by `parseLottieDocument` on
  the way in, not hardcoded) — a parse→write round-trip reproduces the same path.
  - Files changed: `lib/src/bridge/lottie_mapping.dart`
  - Verified by: `test/lottie_mapping_test.dart` (hand-built document round-trip, both static and
    animated-with-easing properties)

- Task 3.1: `ExportImportMode` + `detectMode`/`_hasSweepShape` (`lib/src/ui/lottie/lottie_import.dart`,
  ...
  - Files changed: `lib/src/ui/lottie/lottie_import.dart` (new)
  - Verified by: `test/lottie_import_test.dart` against the real `ASHES.json` (detects
...
- Task 3.2: `LayerPreview`/`LayerPreviewStatus`/`ImportPreview.build` — Full Canvas branch. Precomp
  ...
  own real `id`, e.g. `"comp_0"`); Lottie `parent` indices resolve to `LayerPreview.resolvedParent`
  within the same composition.
  - Files changed: `lib/src/ui/lottie/lottie_import.dart`
  - Verified by: `test/lottie_import_test.dart` (A1 all-clean, A2 mixed/all-unsupported, A3 precomp
...
- Task 3.3: Playback Viewport branch — root-level sweep-shaped layers become scenes
  ...
  compromise.
  - Files changed: `lib/src/ui/lottie/lottie_import.dart`
  - Verified by: `test/lottie_import_test.dart` against real `ASHES.json` (2 real scenes detected,
...
- **A real bug caught by testing, not by inspection**: `LottieLayer.unsupportedReason` was
  originally only computed by the JSON parser (`_layerFromJson`'s own `_unsupportedReasonFor` call)
  — any `LottieLayer` built directly (as several of `lottie_import_test.dart`'s own hand-built test
  ...
  by moving the type->reason classification into `LottieLayer`'s own constructor as a default,
...
- **Small, disclosed refinement to Phase 2's `LottieAsset`** (Task 3.2's F2 concern required it):
  added `LottieAsset.fileFound` (nullable bool), populated only by `parseLottieDocument` (which has
...
- **A disclosed addition beyond `03-specifications.md`'s literal `LayerPreview` field list**:
  ...
  make Lottie `parent`-index resolution actually implementable without `commitImport` re-walking
  the whole `LottieDocument` a second time. A parent index pointing at a precomp layer (rather than
...
- Task 4.1/4.2: `commitImport` — creates real `EditorLayer`s from clean `LayerPreview`s, two-pass
  ...
  - Files changed: `lib/src/ui/lottie/lottie_import.dart`
  - Verified by: `test/lottie_commit_import_test.dart` — exact numeric checks (a hand-built
...
- Task 4.4: A Lottie vector mask (`LottieMask`) imports as `TextRegion.shape == "polygon"` (never
  `"mask"`, since Lottie masks are always vector — confirmed by Test C2's own reasoning).
  - Verified by: `test/lottie_commit_import_test.dart`
...
- **A real architectural gap found while implementing Task 4.1, not by inspection**:
  ...
  have real tempFolder access) extracts the real bytes (Lottie's real content embeds every image as
...
- **A real unit-conversion bug caught by testing**: Lottie's `s` (scale) is a percentage (100 ==
  100%) and `o` (opacity) is 0-100, while `.comics`'s `scaleX`/`scaleY`/`alpha` are unit fractions
  (1 == 100%/fully opaque) — an early version assigned the raw Lottie values directly. Fixed with
  an explicit `/100` conversion at the one point (`_animFor`) where Lottie values become `.comics`
  values.

- Task 5.1/5.2/5.3/5.4: `buildLottieExport` (both modes), `lib/src/ui/lottie/lottie_export.dart`
  (new). Full Canvas: identity, direct inverse of `commitImport`'s own baking (each `.comics` Anim
  chain -> one Lottie keyframe per Anim, `.end` as the frame point). Playback Viewport: partitions
  ...
  (D2); `TextRegion.shape=="polygon"` -> a real Lottie vector mask (D3); `shape=="mask"` (raster)
  is skipped with the disclosed-limitation default, not guessed at.
  - Files changed: `lib/src/ui/lottie/lottie_export.dart` (new)
  - Verified by: `test/lottie_export_test.dart` — exact keyframe/value checks for both modes, D1-D4
...
core data-mutation logic (`lottie_import.dart`/`lottie_export.dart`) are complete and tested against
...
`samples/sample.lottie`) and (b) Phase 6's actual UI (menu entries, review screen widget) — neither
...
**Started at**: Phase 7, Tasks 7.1-7.4. Wrote `test/lottie_roundtrip_test.dart` (G3, G6, E1) plus a
...
issue) in `lottie_export.dart`/`lottie_import.dart`. Fixing them required several rounds of
...
1. **2-level precomp nesting**: `buildLottieExport`'s Playback Viewport branch re-grouped scene
...
2. **Sweep double-counting**: export wrote each member's already-absolute value as its Lottie "own"
   ...
   absolute value before emitting it as "own" (`_sceneMemberToLottie`/`_localPositionAt`).
...
4. **Uniform-`viewportHeight`-per-scene assumption**: `_buildPlaybackViewport` assumed every scene
   ...
   - **static-layer seed artifact**: `_bakedAnims`' own single-sample path seeds every genuinely-
     static member (no real Lottie keyframes) with a zero-width `Anim(start:0, end:0)` placeholder.
...
5. **`_propertyFromAnims` dropped every `Anim.start`**: found via G3 (Full Canvas), a *different* bug
   ...
   at rest, then ease into `moved` over a narrow window) exported as only 2 Lottie keyframes — one
...
**Also fixed**: `test/lottie_roundtrip_test.dart`'s own comparison helper
...
- Files changed: `lib/src/ui/lottie/lottie_export.dart` (`_scrollRangeOf`, `_sceneMemberToLottie`,
  `_buildPlaybackViewport`, `_propertyFromAnims`), `lib/src/ui/lottie/lottie_import.dart`
  (`_bakedAnims`'s `ownFrames` default), `test/lottie_roundtrip_test.dart` (new, name-based layer
  matching), `test/lottie_export_test.dart` (one existing test's assertions updated — it hardcoded
...
#### Completed
- Task 6.4 (`commitImport`'s controller-level counterpart): `EditorController.commitLottieImport`
  ...
  source asset's base64 `data:` URI (the only shape any real `.lottie` file was ever found to use,
  ...
  - Files changed: `lib/src/ui/controller.dart` (`pickLottieToImport`, `setLottieImportMode`,
    `setLottieScrollSpeed`, `setLottieEasingChoice`, `cancelLottieImport`, `commitLottieImport`,
    `_decodeDataUri`, `exportLottieWithDialog`, plus `lottiePreview`/`lottieImportError` state)
  - Verified by: `test/lottie_controller_test.dart` (new, 10 cases) — including the real bar Task
...
- Task 6.1: two new menu entries, "Import from .lottie…" / "Export to .lottie…" — a dedicated
...
- Task 6.2: `showLottieImportDialog`/`_LottieImportReviewDialog` in a new
  `lib/src/ui/widgets/lottie_import_dialog.dart` — a real triage screen: mode segmented control,
  ...
  preview. Also added `showLottieExportDialog` in the same file — deliberately much simpler (mode +
  scrollSpeed only, no per-layer list), matching `buildLottieExport`'s own "easy direction, no
  ...
  - Files changed: `lib/src/ui/widgets/lottie_import_dialog.dart` (new)
- Task 6.3: the wrong-mode banner — recomputes `detectMode(preview.document)` fresh on every build
  ...
  - Files changed: `lib/src/ui/widgets/lottie_import_dialog.dart`
...
- **`FilePicker.saveFile`'s real behavior, confirmed by reading the plugin's own platform
  ...
  all call `FilePickerUtils.saveBytesToFile` internally; mobile requires it). `exportLottieWithDialog`
