# Status: tdd-dot-bodymovin-import-export
...
drafting: **nothing implemented yet** — zero `bodymovin_*.dart` files, no `groupId`/`textRegion`/
...
land first, then pure Bodymovin JSON I/O (Phase 2, independently testable), then the mode-aware import
...
Context for how Specifications got here: v1.1 (real-data investigation found Bodymovin `parent` field
...
(`samples/sample_v2012.comics_unzip`, `samples/sample_playback_viewport.Bodymovin_unzip` — confirmed a
genuinely different file from `samples/sample.Bodymovin` by content hash). Retroactively validates
...
(`.comics → .Bodymovin → .comics`) — corrected to `.Bodymovin → .comics → .Bodymovin`, matching G6.
...
- [x] Specifications extended (2026-08-08) — v1.3, `ExportImportMode` enum threaded through
      `ImportPreview`/`buildBodymovinExport`, mode-branched Data Flow
...
- [x] Phase 2 done (2026-08-08) — pure Bodymovin JSON model + parse/write (`lib/src/bridge
      /bodymovin_mapping.dart`, new): `BodymovinDocument`/`BodymovinLayer`/`BodymovinAsset`/`BodymovinMask`/
      `BodymovinTransform`/`BodymovinProperty`/`BodymovinKeyframe`, `parseBodymovinDocument`/
      `writeBodymovinDocument`. Re-verified `BodymovinMask`'s simplified real-data shape (all 6 real
      ...
      `samples/sample.Bodymovin` (real zip, end-to-end), `ASHES.json`/`THE CHASE.json` (real JSON,
      direct parse). New file: `test/bodymovin_mapping_test.dart` (10 cases).
- [x] Phase 3 done (2026-08-08) — `ExportImportMode`/`detectMode`, `LayerPreview`/
      `ImportPreview.build` for both modes (`lib/src/ui/bodymovin/bodymovin_import.dart`, new). Caught
      and fixed a real bug via testing: `BodymovinLayer.unsupportedReason` was only ever correctly
      computed by the JSON parser, silently defaulting to "supported" for any hand-built
      `BodymovinLayer` — fixed by moving the classification into the constructor itself. Added
      `BodymovinAsset.fileFound` (a Phase 2 refinement) after discovering both real samples embed
      every image as a base64 data URI, never an external file reference. New test file
      `test/bodymovin_import_test.dart` (12 cases), including real-fixture validation (`ASHES.json`:
...
- [x] Phase 4 done (2026-08-08) — `commitImport` (both modes) + `EasingChoice`/`TextRegion` import,
      all in `bodymovin_import.dart`. **Found a real architectural gap while implementing this**:
      ...
      cluttering output vs. real hand-authored content; (2) a unit-conversion bug (Bodymovin's
      0-100/percent scale vs. `.comics`'s 0-1/unit-fraction convention). New test file
      `test/bodymovin_commit_import_test.dart` (8 cases). Full suite: 460/460 passing, `flutter
      analyze` clean.
- [x] Phase 5 done (2026-08-08) — `buildBodymovinExport`, both modes (`lib/src/ui/bodymovin
      /bodymovin_export.dart`, new): Full Canvas is the direct inverse of Task 4.1's baking; Playback
      ...
      polygon->real Bodymovin mask, raster mask skipped with the disclosed D3 default. **Caught a real
      ...
      New test file `test/bodymovin_export_test.dart` (7 cases, including an export->re-import
...
- [x] Phase 7 done (2026-08-08) — real-fixture, real-zip-level round-trip integration tests
      (`test/bodymovin_roundtrip_test.dart`: G3 Full Canvas via `sample_v2012.comics_unzip`, G6
      Playback Viewport via `sample_playback_viewport.Bodymovin_unzip`/`ASHES.json`, E1 via
      `samples/sample.Bodymovin`). All 3 failed on first run, for 5 real bugs found and fixed in
      `bodymovin_export.dart`/`bodymovin_import.dart` (2-level precomp nesting, sweep double-counting,
...
- [x] Phase 6 done (2026-08-08) — UI + real image-byte extraction, the flow's last phase. New
      `lib/src/ui/widgets/bodymovin_import_dialog.dart` (review screen: mode override, scrollSpeed/
      ...
      `EditorController` methods (`pickBodymovinToImport`/`setBodymovinImportMode`/
      `setBodymovinScrollSpeed`/`setBodymovinEasingChoice`/`cancelBodymovinImport`/`commitBodymovinImport`/
      `exportBodymovinWithDialog`) wire `commitImport`/`buildBodymovinExport` to real file-picker/
      tempFolder access. Task 6.4 (real image-byte extraction) closes the disclosed gap from Phase
      4: `commitBodymovinImport` decodes each clean layer's base64 `data:` URI source asset and calls
      ...
      `test/bodymovin_controller_test.dart` (10 cases), including a real end-to-end check that an
...
- **Purpose**: a real feature build (import `.Bodymovin` into `apps/comics-editor`, export `.comics`
  documents as `.Bodymovin`), not a research/consolidation flow like its two siblings
  (`tdd-dot-comics-format`, `tdd-dot-bodymovin-format`).
- **Scope is deliberately narrow**: editor-only (no mobile viewer changes), no sound/translation
  I/O (those live outside the Bodymovin JSON entirely in real content), no shape/mask/text Bodymovin
...
N/A — new flow, not forked. Builds directly on `flows/tdd-dot-bodymovin-format`'s research (cited
...
1. **Manual smoke test** of the new UI on a real device/desktop build (Task 6.2/6.3's own stated
   ...
   per `05-implementation-log.md`'s Phase 6 session notes): pick a real `.Bodymovin` file, confirm the
